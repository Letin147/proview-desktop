...
出于隐私保护，仓库不再附带包含本地用户名、环境路径或密钥片段的设置面板截图。

实际界面会提供 DeepSeek、文心、PaddleOCR、百度语音等配置项，并支持“留空则保持现有值”的更新方式。

如果你在文档或 issue 中分享设置界面截图，请先确认已经移除本地用户名、`.env` 路径和所有密钥相关内容。
...
在 API 示例页复制 `api_key` 与 `base_url` 后填入系统即可。不要把包含真实密钥的截图提交到仓库。
...
在接口示例页复制 `API_URL` 与访问令牌后填入系统即可。不要把包含真实令牌的截图提交到仓库。
